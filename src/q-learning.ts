...
import {selectBestAction} from './q-training';
const worker = new Worker(new URL('./q-training.ts', import.meta.url), {
  type: 'module',
});
...
function aiMakeMove(): void {
  if (!isPlayerTurn) {
   // const qValues = qTable[game.getState()];
    //console.log("qvalues: " + qValues);
    //console.log("qTable: " + JSON.stringify(qTable));
   // const bestMove = Object.keys(qValues).reduce((a, b) => qValues[a] > qValues[b] ? a : b);
      const bestMove = selectBestAction(game.getState());
      const moveScores: Score[] = game.getAvailableMoves().map((move): Score => {
      game.makeMove(move);
     // const score = qTable[game.getState()][move];
      game.undoMove();
    ...
    if (
      ...
      game.makeMove(Number(bestMove));
      renderBoard(game.board);
      isPlayerTurn = true;
      checkGameStatus();
      // Call to display AI thought process with scores for each considered move
      renderAIThoughtProcess(game, moveScores,Number( bestMove));
      
      printScores(moveScores);

    } else {
      console.error('Bestmove returned undefined or invalid', bestMove);
    }
  }
}
